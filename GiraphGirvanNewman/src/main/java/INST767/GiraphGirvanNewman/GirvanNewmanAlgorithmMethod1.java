...
public class GirvanNewmanAlgorithmMethod1 extends
BasicComputation<LongWritable, DoubleWritable, FloatWritable, DoubleWritable> {
  ...
  private static final int PART_1 = 8;

  private static final int PART_2 = 8;
  ...
  public static final LongConfOption SOURCE_ID = new LongConfOption(
      "GirvanNewmanAlgorithm.sourceId", 0, "The shortest paths id");
  ...
  /** Class logger */
  private static final Logger LOG = Logger.getLogger(GirvanNewmanAlgorithmMethod1.class);
  ...
  public void preSuperstep() {
    ...
    if (getSuperstep() == 0) {
      System.out.println("Max counters are: "
          + getConf().getInt("mapreduce.job.counters.limit", 500));
      getConf().setInt("mapreduce.job.counters.limit", 500);
      getConf().setInt("mapreduce.job.counters.max", 500);
      setMaxSuperSteps();
      try {
        for (int i = 0; i < 7; i++) {
          sourceNodes.add(i + 1);
        }
        readSourceNodesFromHDFS("SourceNodes");
      } catch (IOException e) {
        e.printStackTrace();
      }
